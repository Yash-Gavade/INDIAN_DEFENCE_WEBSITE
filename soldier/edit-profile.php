...
<?php  
    ...
    $sql = "SELECT * FROM SOLDIERS WHERE email= '$session_email' ";
    ...
    while( $rows = mysqli_fetch_assoc($result) ){
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( empty($_REQUEST["RANK"]) ){
                $RANKErr = "<p style='color:red'> * RANK is required</p>";
            }else {
                $RANK = $_REQUEST["RANK"];
            }
            ...
            if( !empty($SSNO) && !empty($RNO) && !empty($name) && !empty($email) && !empty($dob) && !empty($RANK)&& !empty($gender) && !empty($pass) && !empty($LOCATION) && !empty($salary) ){
                ...
                if( mysqli_num_rows($r) > 0 ){
                    ...
                    $sql = "UPDATE SOLDIERS SET SSNO= '$SSNO', RNO = '$RNO' ,name = '$name', email = '$email',  dob = '$dob', RANK = '$RANK' ,gender= '$gender', LOCATION = '$LOCATION' WHERE email='$_SESSION[email_emp]' ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
